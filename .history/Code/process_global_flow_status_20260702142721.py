...
def run_label(water_ratio_threshold, min_connected_water_cells, perennial_month_threshold, perennial_ratio_threshold):
    return (
        f"water_ratio_{water_ratio_threshold:.2f}_"
        f"min_conn_{min_connected_water_cells}_"
            ...
            for out_row_start in range(0, out_height, stripe_output_rows):
                ...
                for out_row in range(out_rows):
                    for out_col in range(out_width):
                        if water_count[out_row, out_col] < 20:
                            continue
...
def main():
    tile_paths = global_tile_paths(SOURCE_DIR)
    if not tile_paths:
