...
"""Create a China flow-status mosaic GeoTIFF and PNG from processed tiles."""
...
def main():
    ...
    print(f"Merging {len(tile_paths)} tiles from {run_dir.name}")
    ...
    save_classified_png(flow_status, MOSAIC_PNG)

    print(f"Saved mosaic GeoTIFF: {MOSAIC_TIF}")
    print(f"Saved full-resolution PNG: {MOSAIC_PNG}")
