...
"""Create a global flow-status mosaic GeoTIFF and PNG from processed tiles."""
...
OUTPUT_DIR = PROJECT_ROOT / "Output" / "GlobalFlowStatusMosaic"
...
def main():
    ...
    mosaic_dir = OUTPUT_DIR / run_dir.name
    mosaic_dir.mkdir(parents=True, exist_ok=True)
    OUTPUT_TIF = mosaic_dir / f"global_flow_status_factor33_{run_dir.name}.tif"
    OUTPUT_PNG = mosaic_dir / f"global_flow_status_factor33_{run_dir.name}.png"
    if OUTPUT_TIF.exists() and OUTPUT_PNG.exists():
        print(f"Plot outputs already exist, skipping: {mosaic_dir}")
        return

    print(f"Merging {len(tile_paths)} global flow_status tiles at full resolution from {run_dir.name}")
